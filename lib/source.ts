 ...
 * the Next.js/Nuxt demos (it is CMS-side, not framework-side): per-page locale
 * drives the language facet; section/difficulty become filters. The binding
 * adds the `language` and `site` filters automatically.
 */

import { ContentItem } from "scolta";
import type { ScoltaContentSource } from "scolta-astro/core";
