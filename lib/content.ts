...
// namespace (matching the node: imports above). Works on 4.x too.
import * as yaml from "js-yaml";

// Resolve the corpus dir. The module-relative path is correct when content.ts
// runs directly (the index build scripts), but a bundled server rewrites
// import.meta.url, so the relative `../content` no longer points at the
// project. Fall back to the project root (cwd) — content/ is shipped there — so
// the same loader works both at build time and inside the Astro dev server.
...
const ABOUT_BODY =
  ...
  "search platform, on a content-rich technical reference site built with Astro.</p>" +
...
// -- memoized helpers for the Astro app -------------------------------------
...
/** All catch-all params (slug arrays without slashes), e.g. for getStaticPaths. */
