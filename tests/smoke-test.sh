...
# YAML corpus through lib/content.ts. That loader is what breaks on a js-yaml 5
...
#
# Third, the index assertions are served over HTTP rather than read off disk on
# purpose. This app serves dist/client, while the build writes the Scolta assets
# and the Pagefind index into public/; the postbuild `sync-static` step is what
# copies them across. Checking the files in public/ would pass even if that step
# never ran and the deployed site served nothing. Requesting them from the
# running server is what proves the served site reflects the built index.
set -euo pipefail
...
SERVER_LOG="$(mktemp -t astro-smoke-XXXXXX.log)"
...
# The runtime the search widget loads. It reaches the served site only via the
# postbuild sync-static step, so a 404 here means the build produced an index the
# deployed site cannot use.
echo "==> Checking the synced Scolta runtime assets..."
RUNTIME_CODE=$(await_success "${BASE_URL}/scolta/js/scolta.js" 10) \
  || fail_status "Scolta runtime asset (sync-static did not reach dist/client)" \
