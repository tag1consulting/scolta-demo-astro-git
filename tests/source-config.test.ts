/** Content source mapping + config round-trip + AI handler wiring. */

import { describe, expect, it } from "vitest";
import { ContentItem } from "scolta";
import { createScoltaApi, AstroScoltaConfig } from "scolta-astro/core";
...
describe("AI handler logic (createScoltaApi)", () => {
  it("expandQuery degrades gracefully when AI is unconfigured", async () => {
    ...
    const noAiConfig = AstroScoltaConfig.fromObject({
      ...scoltaConfigInit,
  ...
  it("health reflects saved scoring when healthDetail is on", async () => {
    const detailed = AstroScoltaConfig.fromObject({ ...scoltaConfigInit, healthDetail: true });
